...
let activePlayer = "X";
//This array stores an arraye of moves. We use this to determine win conditions.
let selectedSquares = [];
...
function checkWinConditions() {
    ...
    //Thsi function checks if an array includes 3 strings. It is used to check for
    //each win condition.
...
function drawWinLine(coordX1, coordY1, coordX2, coordY2) {
    ...
    function animateLineDrawing() {
        ...
        if (x1 <= x2 && y1 >= y2) {
            if (x < x2) { x += 10;} 
            if (y > y2) { y -= 10;}
            if (x >= x2 && y <= y2) { concelAnimationFrame(animationLoop);}
        }
    }
